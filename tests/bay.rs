...
fn linked_bay_runs_two_real_elf_nodes_on_a_can_hub() {
    ...
    let board = |name: &str| {
        json!({
            ...
            "memory": {"flash_base": 134217728, "flash_size": 524288, "bootloader_size": 16384, "slot_a_base": 134234112, "slot_a_size": 475136, "erase_size": 2048, "write_alignment": 8, "sedsnet_pool": 4096},
    ...
    for name in ["a", "b"] {
        ...
        .unwrap();
        fs::write(root.join(name).join("build/fw.elf"), b"\x7fELF").unwrap();
    }
