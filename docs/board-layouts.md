...
At the bay-topology level, `reboots` entries select a node and a one-based
`after_sample`. The reset retains the node's physical flash and keeps other
nodes running. Persistence layouts should probe both a boot-only restored value
and any later network-converged value; see `docs/usage.md` for a complete example.
